...
async function mergeAndUpload() {
  const shisData = await downloadFile(
    config.data.sources.sexualHealthInformationServices.url,
    config.data.sources.sexualHealthInformationServices.filename
  );
  const csu25Data = await downloadFile(
    config.data.sources.chlamydiaScreeningUnder25s.url,
    config.data.sources.chlamydiaScreeningUnder25s.filename
  );
