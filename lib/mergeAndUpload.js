...
const whitelistedProperties = require('./utils/whitelist');

async function downloadFileAndEnrich(toEnrich, enrichmentData) {
  const downloadedData = await downloadFile(toEnrich);
  return enrich(downloadedData, enrichmentData, whitelistedProperties, toEnrich.description);
}
...
async function mergeAndUpload() {
  ...
  const transformedData = transform(mergedData);
  ...
  reportObj['Transformed data Object Count'] = transformedData.length;

  await saveDataAndUploadToAzure(transformedData);
  log.info(reportObj, 'Merging and transforming dataset processing report');
}
