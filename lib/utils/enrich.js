...
function enrich(ids, enrichmentData, dataDescription) {
  ...
  log.info(`Enriching dataset for ${dataDescription} took: ${prettyHrtime(endTime)}`);
  log.info(enrichmentReportObject, `${dataDescription} enrichment processing report`);
  return enrichedData;
}
